...
def solve_rbc1d(Ny=100, Ra=1708, Pr=1, alpha=3.14, plot=True):
    ...
    # -------------------- construct matrix  ------------------------
    L2d = UU * 1.0j * alpha + nu * (alpha ** 2 * I - D2y)
    K2d = UU * 1.0j * alpha + kappa * (alpha ** 2 * I - D2y)
    ...
    if plot:
        ...
        ax1.plot(
            np.abs(evecs[0 * N : 1 * N, -1:]), y, marker="", color=blue, label=r"$|u|$"
        )
        ax1.plot(
            np.abs(evecs[1 * N : 2 * N, -1:]), y, marker="", color=red, label=r"$|v|$"
        )
        ...
        ax2.plot(
            np.abs(evecs[3 * N : 4 * N, -1:]), y, marker="", color=yel, label=r"$|T|$"
        )
        ax2.legend()
        plt.tight_layout()
        figname = "rbc1d.png"
